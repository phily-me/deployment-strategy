...
cleanup_docker_dev() {
    ...
    # Clean up any leftover wrapper directory
    if [[ -d "$project_root/$DIRENV_BIN_DIR" ]]; then
        rm -rf "$project_root/$DIRENV_BIN_DIR"
        log_debug "Removed old wrapper directory: $project_root/$DIRENV_BIN_DIR"
    fi
